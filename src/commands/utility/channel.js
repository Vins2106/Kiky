...
exports.run = async (client, message, args, color, prefix, serverQueue, url, searchString, ytdl, YouTube, play, chunk, handleVideo, youtube, queue) => {
    ...
    const channel = await fetch.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${name}&key=${client.config.yt}&maxResults=1&type=channel`)
    ...
    const data = await fetch.get(`https://www.googleapis.com/youtube/v3/channels?part=snippet,contentDetails,statistics,brandingSettings&id=${channel.body.items[0].id.channelId}&key=${client.config.yt}`)
  ...
  .setAuthor(channel.body.items[0].snippet.channelTitle + ' Channel', channel.body.items[0].snippet.thumbnails.medium.url)
  .setColor(color)
  .setTitle(parseInt(data.body.items[0].statistics.subscriberCount).toLocaleString() + ' Subscribers')
  ...
  .addField(`Created At`, new Date(channel.body.items[0].snippet.publishedAt).toDateString(), true)
